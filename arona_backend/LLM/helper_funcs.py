...
class HelperFn:
    ...
    def wait_for_element_css(self, CSS_SELECTOR, timeout=20):
        try:
            WebDriverWait(self.driver, timeout).until(EC.presence_of_element_located((By.CSS_SELECTOR, CSS_SELECTOR)))
            logging.info("# Element '%s' is present." % CSS_SELECTOR)
        except TimeoutException:
            logging.error("# Element '%s' is not present." % CSS_SELECTOR)
